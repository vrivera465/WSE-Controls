...
// Button Pins 
const int FB1 = A9;  // Button for Flexion (Corresponds to EMG1placement)
const int AB2 = A8;  // Button for Aduction (Corresponds to EMG2placement)
...
//Prints the values in matrix C
  ...
  float flexInitial; //Creates the variable for the initial postion of the arm 
  float addInitial; //Creates the variable for the initial postion of the arm 
 ...
 if(digitalRead(FB1) == HIGH) // if the button is pressed 
 {
  ...
  //Prints the current pitch arm angle 
  Serial.print("pitchArm: "); Serial.print(pitchArm, 2); 
  flexInitial = pitchArm; //defines the variable to be equal to the current arm position
  Serial.print ("flexInitial: "); Serial.print(flexInitial, 2); // Prints the value of flexInitial
    if (abs((flexInitial+30)-pitchArm) <= 5){ //if the initial postion of the arm is 30 degrees less than the current postion
    analogWrite(MD1pos, 0); //The motors stop 
    analogWrite(MD1neg, 0); //The motors stop   
   } 
 }
...
{ 
  ...
  //Prints the current pitch arm angle
  Serial.print("yawArm: "); Serial.print(yawArm, 2);
  addInitial = yawArm;  //defines the variable to be equal to the current arm position
  Serial.print("addInitial: "); Serial.print(addInitial, 2);
  
    if(abs((addInitial-2)-yawArm) <= .5){ //if the initial postion of the arm is 30 degrees less than the current postion
    analogWrite(MD2pos, 0); //The motors stop
    analogWrite(MD2neg, 0); //The motors stop 
    }
